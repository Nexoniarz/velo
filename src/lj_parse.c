...
/* Velo backward-compat: accept '{' (Velo) or lua_tok (Lua). */
static void lex_check_block_open(LexState *ls, LexToken lua_tok)
{
  if (ls->tok == '{') {
    lj_lex_next(ls);
  } else {
    lex_check(ls, lua_tok);
  }
}

/* Velo backward-compat: accept '}' (Velo) or TK_end (Lua) as block close. */
static void lex_match_block_close(LexState *ls, LexToken who, BCLine line)
{
  if (!lex_opt(ls, '}'))
    lex_match(ls, TK_end, who, line);
}
...
/* Like lex_str but also accepts keywords as field names (e.g. obj.new, obj.class). */
static GCstr *lex_field_name(LexState *ls)
{
  GCstr *s;
  if (ls->tok != TK_name && (LJ_52 || ls->tok != TK_goto)) {
    /* Accept any reserved keyword as a field name — tokval holds the string. */
    if (ls->tok < TK_OFS || !tvisstr(&ls->tokval))
      err_token(ls, TK_name);
  }
  s = strV(&ls->tokval);
  lj_lex_next(ls);
...
static void expr_field(LexState *ls, ExpDesc *v)
{
  ...
  key.u.sval = lex_field_name(ls);  /* Accept keywords as field names (e.g. obj.new). */
  expr_index(fs, v, &key);
}
...
static void expr_table(LexState *ls, ExpDesc *e)
{
  ...
  uint32_t narr = 0;  /* First array index (0-based). */
...
static void parse_body(LexState *ls, ExpDesc *e, int needself, BCLine line)
{
  ...
  lex_opt(ls, '{');  /* Velo: '{'; Lua compat: body starts right after ')'. */
  parse_chunk(ls);
  lex_match_block_close(ls, TK_function, line);
...
static void expr_primary(LexState *ls, ExpDesc *v)
{
  ...
  if (ls->tok == '(') {
    ...
    var_lookup(ls, v);
  } else if (ls->tok == TK_this) {
    /* 'this' is an alias for the 'self' variable. */
    var_lookup_(ls->fs, lj_str_newlit(ls->L, "self"), v, 1);
    lj_lex_next(ls);
  } else if (ls->tok == TK_new) {
    /* 'new ClassName(args)' desugars to 'ClassName.new(args)'. */
    ExpDesc key;
    lj_lex_next(ls);  /* Skip 'new'. */
    var_lookup_(ls->fs, lex_str(ls), v, 1);  /* Read class name. */
    expr_toanyreg(ls->fs, v);
    expr_init(&key, VKSTR, 0);
    key.u.sval = lj_str_newlit(ls->L, "new");
    expr_index(ls->fs, v, &key);
  } else {
...
/* Parse compound assignment: x op= rhs  or  x++ */
static void parse_compound_assign(LexState *ls, ExpDesc *lhs)
{
  FuncState *fs = ls->fs;
  LexToken op = ls->tok;
  BinOpr binopr;
  ExpDesc rhs, tmp_val, lhs_store;

  switch (op) {
  case TK_addassign: binopr = OPR_ADD; break;
  case TK_subassign: binopr = OPR_SUB; break;
  case TK_mulassign: binopr = OPR_MUL; break;
  case TK_divassign: binopr = OPR_DIV; break;
  case TK_incr:      binopr = OPR_ADD; break;
  case TK_decr:      binopr = OPR_SUB; break;
  default: lj_lex_error(ls, ls->tok, LJ_ERR_XSYNTAX); return;
  }

  lj_lex_next(ls);  /* Skip operator token. */

  /* RHS: explicit expression or implicit 1 for ++/-- */
  if (op == TK_incr || op == TK_decr) {
    expr_init(&rhs, VKNUM, 0);
    setintV(&rhs.u.nval, 1);
  } else {
    expr(ls, &rhs);
  }

  if (lhs->k == VINDEXED) {
    /* For indexed LHS, manually emit TGET to avoid corrupting table register. */
    BCReg table_reg = lhs->u.s.info;
    BCReg key_aux = lhs->u.s.aux;
    BCReg val_reg = fs->freereg;
    bcreg_reserve(fs, 1);
    if ((int32_t)key_aux < 0) {
      bcemit_ABC(fs, BC_TGETS, val_reg, table_reg, ~key_aux);
    } else if (key_aux > BCMAX_C) {
      bcemit_ABC(fs, BC_TGETB, val_reg, table_reg, key_aux-(BCMAX_C+1));
    } else {
      bcemit_ABC(fs, BC_TGETV, val_reg, table_reg, key_aux);
    }
    expr_init(&tmp_val, VNONRELOC, val_reg);
    bcemit_binop_left(fs, binopr, &tmp_val);
    bcemit_binop(fs, binopr, &tmp_val, &rhs);
    BCReg result_reg = expr_toanyreg(fs, &tmp_val);
    if ((int32_t)key_aux < 0) {
      bcemit_ABC(fs, BC_TSETS, result_reg, table_reg, ~key_aux);
    } else if (key_aux > BCMAX_C) {
      bcemit_ABC(fs, BC_TSETB, result_reg, table_reg, key_aux-(BCMAX_C+1));
    } else {
      bcemit_ABC(fs, BC_TSETV, result_reg, table_reg, key_aux);
    }
  } else {
    /* For VLOCAL / VGLOBAL / VUPVAL: standard approach. */
    lhs_store = *lhs;
    tmp_val = *lhs;
    bcemit_binop_left(fs, binopr, &tmp_val);
    bcemit_binop(fs, binopr, &tmp_val, &rhs);
    bcemit_store(fs, &lhs_store, &tmp_val);
  }
}
...
static void parse_call_assign(LexState *ls)
{
  ...
  } else if (ls->tok == TK_addassign || ls->tok == TK_subassign ||
             ls->tok == TK_mulassign || ls->tok == TK_divassign ||
             ls->tok == TK_incr || ls->tok == TK_decr) {
    parse_compound_assign(ls, &vl.v);
...
/* Parse class constructor body: auto-injects self = setmetatable({}, ClassName). */
static void parse_class_ctor(LexState *ls, ExpDesc *e, BCLine line, GCstr *clsname)
{
  FuncState cfs, *pfs = ls->fs;
  FuncScope bl;
  GCproto *pt;
  ptrdiff_t oldbase = pfs->bcbase - ls->bcstack;

  fs_init(ls, &cfs);
  fscope_begin(&cfs, &bl, 0);
  cfs.linedefined = line;
  cfs.numparams = (uint8_t)parse_params(ls, 0);
  cfs.bcbase = pfs->bcbase + pfs->pc;
  cfs.bclim = pfs->bclim - pfs->pc;
  bcemit_AD(&cfs, BC_FUNCF, 0, 0);
  lex_opt(ls, '{');  /* Velo: '{'; Lua compat: body starts right after ')'. */

  /* Inject preamble: local self = setmetatable({}, ClassName) */
  {
    BCReg call_base = cfs.freereg;
    BCReg nregs = (BCReg)(1 + ls->fr2 + 1 + 1);
    bcreg_reserve(&cfs, nregs);

    {
      ExpDesc smt;
      expr_init(&smt, VKSTR, 0);
      smt.u.sval = lj_parse_keepstr(ls, "setmetatable", 12);
      bcemit_AD(&cfs, BC_GGET, call_base, const_str(&cfs, &smt));
    }

    {
      BCReg arg1 = call_base + 1 + ls->fr2;
      BCReg arg2 = arg1 + 1;
      ExpDesc cls;

      bcemit_AD(&cfs, BC_TNEW, arg1, 0);

      var_lookup_(ls->fs, clsname, &cls, 1);
      expr_toreg(ls->fs, &cls, arg2);

      bcemit_ABC(&cfs, BC_CALL, call_base, 2, 3);

      cfs.freereg = call_base + 1;
  ...
  if (!(cfs.flags & PROTO_HAS_RETURN)) {
    bcemit_AD(&cfs, BC_RET1, (BCReg)cfs.numparams, 2);
    cfs.flags |= PROTO_HAS_RETURN;
  }
...
static void parse_class(LexState *ls, BCLine line)
{
  FuncState *fs = ls->fs;
  GCstr *clsname;
  ExpDesc store_target, fn_exp;
  BCReg clsreg;

  lj_lex_next(ls);  /* Skip 'class'. */
  clsname = lex_str(ls);

  /* Emit: local ClassName = {} */
  var_new(ls, 0, clsname);
  clsreg = fs->freereg;
  bcreg_reserve(fs, 1);
  var_add(ls, 1);
  bcemit_AD(fs, BC_TNEW, clsreg, 0);

  /* Emit: ClassName.__index = __velo_make_index(ClassName)
  ** This sets up a smart __index that auto-passes the instance as 'self'
  ** when calling dot-methods: p.method(args) works like p:method(args). */
  {
    BCReg fn_reg = fs->freereg;
    BCReg nregs = (BCReg)(1 + ls->fr2 + 1);
    ExpDesc make_idx;
    ExpDesc idx_key;

    bcreg_reserve(fs, nregs);
    expr_init(&make_idx, VKSTR, 0);
    make_idx.u.sval = lj_parse_keepstr(ls, "__velo_make_index", 17);
    bcemit_AD(fs, BC_GGET, fn_reg, const_str(fs, &make_idx));
    bcemit_AD(fs, BC_MOV, fn_reg + 1 + ls->fr2, clsreg);
    bcemit_ABC(fs, BC_CALL, fn_reg, 2, 2);  /* 1 result, 1 arg */
    fs->freereg = fn_reg + 1;  /* result in fn_reg */

    expr_init(&idx_key, VKSTR, 0);
    idx_key.u.sval = lj_parse_keepstr(ls, "__index", 7);
    bcemit_ABC(fs, BC_TSETS, fn_reg, clsreg, const_str(fs, &idx_key));
    fs->freereg = fn_reg;  /* free temp */
  }

  lex_check(ls, '{');

  while (ls->tok != '}' && ls->tok != TK_eof) {
    BCLine meth_line = ls->linenumber;
    GCstr *meth_name;
    ExpDesc meth_key;

    lex_check(ls, TK_function);
    meth_name = lex_str(ls);

    /* Set up VINDEXED store target: clsreg["new"] or clsreg[methname] */
    expr_init(&store_target, VNONRELOC, clsreg);
    expr_toanyreg(fs, &store_target);
    expr_init(&meth_key, VKSTR, 0);
    if (meth_name == clsname) {
      meth_key.u.sval = lj_parse_keepstr(ls, "new", 3);
    } else {
      meth_key.u.sval = meth_name;
    }
    expr_index(fs, &store_target, &meth_key);

    if (meth_name == clsname) {
      parse_class_ctor(ls, &fn_exp, meth_line, clsname);
    } else {
      parse_body(ls, &fn_exp, 1, meth_line);
    }

    bcemit_store(fs, &store_target, &fn_exp);
    lex_opt(ls, ';');
  }

  lex_match(ls, '}', TK_class, line);
}
...
static void parse_while(LexState *ls, BCLine line)
{
  ...
  lex_check_block_open(ls, TK_do);
  loop = bcemit_AD(fs, BC_LOOP, fs->nactvar, 0);
  parse_block(ls);
  jmp_patch(fs, bcemit_jmp(fs), start);
  lex_match_block_close(ls, TK_while, line);
  fscope_end(fs);
...
static void parse_repeat(LexState *ls, BCLine line)
{
  ...
  /* Lua compat: 'repeat' body has no opening delimiter; Velo uses '{'. */
  lex_opt(ls, '{');
...
static void parse_for(LexState *ls, BCLine line)
{
  ...
  lex_match_block_close(ls, TK_for, line);
  fscope_end(fs);  /* Resolve break list. */
}

/* Parse condition and block: accepts '{' (Velo) or 'then' (Lua compat). */
static BCPos parse_then(LexState *ls)
{
  BCPos condexit;
  lj_lex_next(ls);  /* Skip 'if' or 'elseif'. */
  condexit = expr_cond(ls);
  lex_check_block_open(ls, TK_then);
  parse_block(ls);
  /* In Velo style consume '}'; in Lua style the block ends at else/elseif/end. */
  lex_opt(ls, '}');
  return condexit;
}
...
static void parse_if(LexState *ls, BCLine line)
{
  ...
  flist = parse_then(ls);
  /* Handle 'elseif' or 'else if' chains. */
  for (;;) {
    if (ls->tok == TK_elseif) {
      jmp_append(fs, &escapelist, bcemit_jmp(fs));
      jmp_tohere(fs, flist);
      flist = parse_then(ls);
    } else if (ls->tok == TK_else) {
      BCLine else_line = ls->linenumber;
      /* 'else if' on the SAME LINE is a chained condition (like elseif).
       * 'else' followed by 'if' on a NEW LINE is an else block that starts
       * with an if statement — NOT a chain. After lj_lex_lookahead,
       * ls->linenumber is updated to the line of the lookahead token. */
      if (lj_lex_lookahead(ls) == TK_if && ls->linenumber == else_line) {
        lj_lex_next(ls);  /* Skip 'else', tok is now 'if'. */
        jmp_append(fs, &escapelist, bcemit_jmp(fs));
        jmp_tohere(fs, flist);
        flist = parse_then(ls);
      } else {
  ...
  if (ls->tok == TK_else) {  /* Parse optional 'else' block. */
    ...
    /* Velo: 'else {'; Lua compat: 'else' directly followed by body. */
    lex_opt(ls, '{');
    parse_block(ls);
    lex_opt(ls, '}');
  } else {
  ...
  lex_opt(ls, TK_end);  /* Lua compat: consume final 'end' if present. */
}
...
static int parse_stmt(LexState *ls)
{
  ...
  case TK_do:
    ...
    lex_check_block_open(ls, TK_do);
    parse_block(ls);
    lex_match_block_close(ls, TK_do, line);
    break;
  case TK_for:
    parse_for(ls, line);
    break;
