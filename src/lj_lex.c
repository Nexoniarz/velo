...
static LexToken lex_scan(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
    ...
    case '+':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_addassign; }
      if (ls->c == '+') { lex_next(ls); return TK_incr; }
      return '+';
    case '*':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_mulassign; }
      return '*';
    case '/':
      lex_next(ls);
      if (ls->c == '/') {  /* Single-line comment "//...". */
	while (!lex_iseol(ls) && ls->c != LEX_EOF) lex_next(ls);
	continue;
      }
      if (ls->c == '*') {  /* Multi-line block comment. */
	lex_next(ls);
	for (;;) {
	  if (ls->c == LEX_EOF) lj_lex_error(ls, TK_eof, LJ_ERR_XLCOM);
	  if (ls->c == '*') {
	    lex_next(ls);
	    if (ls->c == '/') { lex_next(ls); break; }
	  } else {
	    if (lex_iseol(ls)) lex_newline(ls); else lex_next(ls);
	  }
	}
	continue;
      ...
      if (ls->c == '=') { lex_next(ls); return TK_divassign; }
      return '/';
    case '-':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_subassign; }
      if (ls->c == '-') { lex_next(ls); return TK_decr; }
      return '-';
    case '[': {
